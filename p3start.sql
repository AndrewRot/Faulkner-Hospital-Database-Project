...
Select PC.PathOrder, L.LocationName, L.FloorID, PC.PathID
from Location L, PathContains PC
where L.LocationID = PC.LocationID
and PC.PathID = 
	(SELECT PC.PathID
	FROM PathContains PC
	Group By PathID
	Having count(PC.PathID) =
		( SELECT Min(CNT) As Min
	  	FROM (
	  		SELECT PC.PathID, count(PC.LocationID) As CNT
	  		FROM PathContains PC, Path P
	  		where P.PathStart = 'Radiology' and P.PathEnd = '5J'
